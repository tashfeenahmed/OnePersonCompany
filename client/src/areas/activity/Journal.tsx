...
export function Journal() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center gap-4 rounded-xl border bg-card p-4">
      ...
      <label className="ml-auto text-sm">Venture <select className="ml-2 rounded-lg border bg-background p-2" value={venture} onChange={e => { setVenture(e.target.value); setOffset(0); }}><option value="">All ventures</option>{state.ventures.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label>
    ...
    <form className="space-y-3 rounded-xl border bg-card p-4" onSubmit={e => { e.preventDefault(); void write(async () => { await call(`/journal${editing ? `/${editing.id}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify({ day: day || data.today, kind, text, ventureId: entryVenture || null }) }); reset(); }); }}>
      ...
      <div className="flex flex-wrap gap-2">
        <select aria-label="Entry type" className="rounded-lg border bg-background p-2 text-sm" value={kind} onChange={e => setKind(e.target.value as JournalKind)}>{JOURNAL_KINDS.map(k => <option key={k} value={k}>{k}</option>)}</select>
        ...
        <select aria-label="Entry venture" className="rounded-lg border bg-background p-2 text-sm" value={entryVenture} onChange={e => setEntryVenture(e.target.value)}><option value="">No venture</option>{state.ventures.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select>
      </div>
