  ...
  return <section className="space-y-4" aria-label="Workflow builder">
    ...
    <Dialog open={!!selected} onOpenChange={value => { if(!value) setEditing(null); }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Edit block</DialogTitle><DialogDescription>{selected ? BLOCK_DESCRIPTIONS[selected.kind] : ""}</DialogDescription></DialogHeader>
      {selected && <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3"><Field label="Run cadence"><select className={control} value={selected.cadence} onChange={e => patch(selected.id,{ cadence:e.target.value as WorkflowBlock["cadence"] })}>{["daily","weekly","monthly"].map(c => <option key={c}>{c}</option>)}</select></Field><Field label="Time limit (minutes)"><Input type="number" min={1} max={240} value={selected.maxMinutes} onChange={e => patch(selected.id,{ maxMinutes:Number(e.target.value) })} /></Field></div>
        {selected.agent && <>
          <Field label="Sub-agent role"><select className={control} value={selected.agent.role} onChange={e => patch(selected.id,{ agent:{ ...selected.agent!,role:e.target.value } })}>{data.roles.map(r => <option value={r.id} key={r.id}>{r.title}</option>)}</select></Field>
