  ...
  return (
    <div className="border-line-soft flex flex-col gap-1 border-b py-2.5 last:border-b-0">
      <div className="flex flex-wrap items-center gap-2">
        <span className={cn("size-[7px] rounded-[2px]", statusTone(job.lastStatus))} aria-hidden />
        <span className="text-[13px] font-medium">{job.name ?? job.id}</span>
        {job.schedule && (
          <span className="text-muted-foreground text-[11.5px]">{job.schedule}</span>
        )}
        {job.enabled === false && (
          <span className="text-muted-foreground text-[11.5px]">disabled</span>
        )}
      </div>
      ...
      {job.lastError && <div className="text-destructive text-[11.5px]">{job.lastError}</div>}
    </div>
  );
}
...
export function RuntimeJobsPanel({ id }: { id: "hermes" | "openclaw" }) {
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <h3 className="text-[13.5px] font-medium">Scheduled jobs</h3>
        <Button
          variant="outline"
          size="sm"
          className="ml-auto"
      ...
      {doc.error && <p className="text-destructive mt-2 text-[12.5px]">{doc.error}</p>}

      {doc.data && (
        <p className="text-muted-foreground mt-1.5 text-[11.5px]">{doc.data.scheduling}</p>
      )}

      {reading && !reading.readable && (
        <div className="bg-card border-line-soft mt-3 rounded-[10px] border p-3.5">
          <p className="text-[12.5px]">Not readable for this runtime.</p>
          <p className="text-muted-foreground mt-1 text-[11.5px]">{reading.note}</p>
        </div>
      )}
      ...
      {reading?.readable && (
        <div className="mt-2">
          {reading.jobs.length === 0 ? (
            <p className="text-muted-foreground text-[12.5px]">{reading.note}</p>
          ) : (
            <>
              <p className="text-muted-foreground mb-1 text-[11.5px]">{reading.note}</p>
              {reading.jobs.map((j) => (
      ...
      {note && <p className="text-muted-foreground mt-2 text-[11.5px]">{note}</p>}
