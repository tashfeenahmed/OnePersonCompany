...
export function VentureJourney({ venture, children }: { venture: Venture; children: ReactNode }) {
  ...
  const doc = request.data, stage = venture.stage, type = venture.businessType ?? null, copy = COPY[stage];
  ...
  const setType = (value: BusinessType | null) => mutate(async () => { await updateVenture(venture.id, { businessType: value, expectedUpdatedAt: venture.updatedAt }); setFilter("all"); request.reload(); });
  ...
  const tasks = journeyTasks(doc.state, stage, type), readiness = journeyReadiness(tasks, doc.state.tasks), profile = doc.state.profile;
  ...
  const brief = `Venture: ${venture.name}. Stage: ${STAGE_LABELS[stage]}. Type: ${businessLabel(type)}. Read its venture journey using venture ID ${venture.id} before making recommendations. Use the saved assumptions and evidence; distinguish unknowns from facts.`;
  ...
  return <div className="venture-journey min-h-0 flex-1 overflow-y-auto px-5 pb-16 md:px-8"><div className="mx-auto w-full max-w-[1220px]">
    ...
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-3 text-xs text-muted-foreground">Business type<select aria-label="Business type" className="journey-select w-auto!" value={type ?? ""} disabled={busy || request.loading} onChange={e => { void setType(e.target.value ? e.target.value as BusinessType : null); }}><option value="">Choose a type</option>{BUSINESS_TYPES.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}</select></label><span className="journey-muted" role="status">{busy ? "Saving…" : doc.updatedAt ? "Plan saved" : "Your plan is ready to start"}</span></div>
    ...
    <div className="journey-grid"><div className="journey-stack">
      ...
      <section className="journey-card"><div className="journey-card-header"><div><h2>{copy.checklist}</h2><p className="journey-muted mt-1">{type ? businessLabel(type) : "Common foundations"} · {readiness.done} complete{readiness.skipped ? ` · ${readiness.skipped} skipped with a reason` : ""}</p></div><button onClick={() => edit({ kind: "custom" })} className="flex items-center gap-1"><Plus className="size-3.5" />Add step</button></div>
      ...
      {(doc.history.length > 0 || doc.reviews.length > 0) && <section className="journey-card"><h2>Decisions & reviews</h2>{doc.history.slice(0, 4).map(h => <div className="journey-name" key={h.id}><p className="text-xs">{STAGE_LABELS[h.fromStage]} → {STAGE_LABELS[h.toStage]}</p><p className="journey-muted text-xs!">{new Date(h.at).toLocaleDateString()}{h.note ? ` · ${h.note}` : ""}</p></div>)}{doc.reviews.slice(0, 5).map(r => <button className="journey-name block w-full text-left text-xs" key={`review-${r.id}`} onClick={() => edit({ kind: "history", id: r.id })}>Review · {new Date(r.at).toLocaleDateString()}<span className="journey-muted block text-xs!">{r.done}/{r.total} complete · {businessLabel(r.businessType)}</span></button>)}<p className="journey-muted mt-3 text-xs!">Recent activity shown. Export the plan for more stage history.</p></section>}
    </aside></div>
    ...
    {editor?.kind === "custom" && <CustomTaskEditor stage={stage} businessType={type} {...formProps} />}
    ...
    {editor?.kind === "review" && <JourneyModal title="Start the next operating review" description="The current checklist and its evidence will be archived. Its steps will reopen for the next review; other stages and business-specific tracks stay saved." close={close}><ErrorNotice error={error} /><div className="journey-modal-footer"><Button variant="ghost" onClick={close}>Cancel</Button><Button disabled={busy} onClick={async () => { if (await save({ kind: "start-review", businessType: type })) close(); }}>Archive & start next review</Button></div></JourneyModal>}
