...
export function QueueControls() {
  ...
  return <section className="border rounded-xl p-3 mb-4 space-y-2">
    <div className="flex flex-wrap gap-3 items-center"><h2 className="font-medium text-sm">Queue controls</h2><button className="border rounded px-2 py-1 text-sm" disabled={!doc.data || busy} onClick={() => void change("/runs/controls", { paused: !doc.data?.paused })}>{doc.data?.paused ? "Resume queue" : "Pause queue"}</button><Link className="underline text-sm" to="/settings?tab=budgets">Usage limits</Link></div>
