...
export class RecoveryBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  render() {
    ...
    return <main className="mx-auto max-w-xl p-8 space-y-4" role="alert">
      ...
      <button className="rounded-lg p-2.5 bg-muted hover:bg-[color-mix(in_oklch,var(--muted),var(--foreground)_6%)] transition-colors disabled:opacity-50" onClick={() => location.reload()}>Reload</button>{" "}
      <button className="rounded-lg p-2.5 bg-muted hover:bg-[color-mix(in_oklch,var(--muted),var(--foreground)_6%)] transition-colors disabled:opacity-50" onClick={() => {
