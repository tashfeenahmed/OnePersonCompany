...
THEN GO OUTSIDE AND FIND THE WORDING. This is the part only you can do.
READ REDDIT THROUGH THE WRAPPER, NEVER FROM reddit.com: this machine is blocked there (page fetches time out, the browser hits a CAPTCHA, old.reddit and the .json endpoints refuse), so a reddit.com URL in web_extract or the browser is a wasted call. Use the \`reddit\` source instead — it reads the Arctic Shift archive and gives every post and comment its date, score and permalink:
- \`opc reddit search --subreddit <name> --q "<words>" --days 90\` — posts in a subreddit matching the words (it needs a subreddit; find the subreddits with a site:reddit.com web search first);
- \`opc reddit comments --subreddit <name> --q "<words>"\` — the comments that say it, which is where the asks usually are;
- \`opc reddit thread --id <post id or reddit.com URL>\` — one thread with its top comments. Any reddit.com link a web search hands you goes here.
Search Reddit that way, Hacker News, the forums and subreddits where this venture's buyers actually are, review sites, question-and-answer pages and the "people also ask" boxes, and collect HOW STRANGERS PHRASE THE NEED — in their words, not yours. For every quote you bring back record: the sentence VERBATIM, the URL, the hostname, the date it was posted, and its upvotes or points where the page shows them. A paraphrase is worth nothing here; a sentence somebody typed is the whole product of this run.
