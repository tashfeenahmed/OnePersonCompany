 ...
 * node answers in the locale of whatever IP asked, and the box it runs on is
 * in Nuremberg — so the same query would quietly return German results from a
 * German datacentre and English ones from here.
 */
export async function search(
  ...
  const target =
    `${url}?q=${encodeURIComponent(query)}&format=json&language=en-US`;
  const started = Date.now();
  let res: Response;
  try {
    res = await fetch(target, {
      headers: {
        /* THE HEADER IS THE ONLY DOOR. See the file header: the same key as a
           `?key=` parameter is refused 401. */
        "x-api-key": key,
...
export const CANNOT = {
  ...
  asked: [
    ...
    },
    {
      ...
      answer: "401. The key is an x-api-key HEADER and nothing else works.",
    },
    {
      asked: "GET /healthz without the key",
      answer: "401 — every path on this node is behind the key, liveness included.",
    },
    {
...
export async function verify(
  ...
  try {
    ...
    */
    if (!answer.results.length && answer.unresponsive.length)
      return {
        ok: false,
        error:
...
export async function collect(
  ...
  const { ready, broken } = accounts.credentialed("searxng", ["key"], reader);
  const url = endpoint();
...
export function borrowKey(reader: string): { url: string; key: string } | null {
  const { ready } = accounts.credentialed("searxng", ["key"], reader);
  const first = ready[0];
  if (!first) return null;
  const key = (first.values.key ?? "").trim();
  return key ? { url: endpoint(), key } : null;
}
